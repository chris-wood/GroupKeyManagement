...
% Result containers
times = zeros(numChildren, numAuthProbs, numKeyProbs, numSims,numSamples);
            ...
            for n = 1:numSims
                ...
                for i = 1:numSamples
                    ...
                    while (nConnected < (numNodes(n) - 1)) % We go until connected == (n-1)
                        ...
                        [parentList, parentCount] = readyParents(aMatrix, cMatrix, authMatrix, maxChildren(childIndex), numNodes(n), kMult);
                    ...
                    times(childIndex,pAuthIndex,pKeyIndex,n,i) = time;
                end
            end
        end
    end
